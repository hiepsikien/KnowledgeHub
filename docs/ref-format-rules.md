...
| Source form | Example |
|-------------|---------|
| Numbered dump (Abdy Williams) | `[1] See Glossary, "College of Instrumental Musicians."` — dump keeps the pointer; publish resolves it from the Glossary chapter onto `span.note` / `notes[].body` and emits `glossary[]` term cards. The Glossary chapter stays visible. |
