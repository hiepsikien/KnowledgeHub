...
## REF/1 v1.8 — per-chapter FOOTNOTES
...
Every `FOOTNOTES:` heading followed by those items is a dump (not only a book-tail section after 40% of the file). `NOTES TO …` essays are still kept as body text. CMS Chế bản shows the note body on hover and in a list at the end of the chapter.
