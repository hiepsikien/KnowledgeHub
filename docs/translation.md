...
Uses the locked mode. Same DeepSeek → Gemini pipeline; writes `segments/chii.json`. The curator UI **Dịch chương** button enqueues the same work on a **background worker pool** (HTTP returns immediately). Pool size is `min_workers`–`max_workers` in Cài đặt (default 1–2). **Dịch chương còn thiếu** queues every chapter whose `completeness` is not `ok` (empty, truncated, incomplete parts, or polish pending — not only chapters with no `final`). The confirm shows a rough token estimate from `segment.words` (old segments missing that field undercount source tokens but still include prompt overhead) plus auto annotate/QA if those Cài đặt flags are on. The dialog refreshes `missing_preview` from GET before confirm; enqueue still follows the live Cài đặt when the worker runs. QA and chú thích use the same queue. Workers claim **draft → annotate → QA** across the queue (all dịch before chú thích before QA); two jobs for the same chapter never run at once. Each job writes `phase` / `detail` as it moves (DeepSeek nháp → lưu nháp → Gemini chỉnh văn). **Hủy job đang chạy** marks jobs `cancelled` and skips the next LLM call (in-flight HTTP may still finish). Boot, stop (Ctrl+C / lifespan), and uvicorn `--reload` mark both `running` and `queued` translation jobs `interrupted` so a restart cannot keep spending tokens. (Saving Python under `src/` with `--reload` therefore drops the rest of the queue.) The chế bản worker uses the same interrupt rule. Cài đặt `max_attempts` (default 2) and `job_timeout_sec` (default 600) cap retries and wall time.
