...
def interrupt_stale_running(*, include_queued: bool = False) -> int:
    """Drop in-flight jobs instead of silently retrying (token guard).

    Running jobs are dropped on worker start. Queued jobs are dropped on start
    (kill -9 leftovers) and on shutdown so a new hub cannot keep spending tokens.
    """
    stale = {"running"}
    if include_queued:
        stale.add("queued")
